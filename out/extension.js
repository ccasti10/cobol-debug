...
const debugGenerator_1 = require("./debugGenerator");
...
function activate(context) {
    ...
    if (workspaceRoot) {
        debugState_1.DebugState.getInstance().initialize(workspaceRoot);
    }
    // Register debug panel (tree view)
    (0, debugPanel_1.registerDebugPanel)(context);
    // Register commands
    registerCommands(context);
}
function registerCommands(context) {
    // Generate Debug Version
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.generateDebugVersion', async () => {
        await generateDebugVersion();
    }));
    // Watch Variable (Current Line)
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.watchVariableCurrentLine', async () => {
        await watchVariable(types_1.DebugMode.CURRENT_LINE);
    }));
    // Watch Variable (BEFORE/AFTER)
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.watchVariableBeforeAfter', async () => {
        await watchVariable(types_1.DebugMode.BEFORE_AFTER);
    }));
    // Track Variable (All Occurrences)
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.trackVariableAllOccurrences', async () => {
        await trackVariableAll();
    }));
    // Trace Paragraph
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.traceParagraph', async () => {
        await traceParagraph();
    }));
    // Debug All Variables in Paragraph
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.debugAllVariablesInParagraph', async () => {
        await debugAllVariablesInParagraph();
    }));
    // Debug Variables in Selection
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.debugVariablesInSelection', async () => {
        await debugVariablesInSelection();
    }));
    // Remove from Debug
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.removeFromDebug', async () => {
        await removeFromDebug();
    }));
    // Clear All Debug Points
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.clearAllDebugPoints', async () => {
        const answer = await vscode.window.showWarningMessage('Are you sure you want to clear all debug points?', 'Yes', 'No');
        if (answer === 'Yes') {
            debugState_1.DebugState.getInstance().clearAll();
            vscode.window.showInformationMessage('All debug points cleared');
        }
    }));
    // Save Configuration
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.saveConfiguration', async () => {
        const saved = await debugState_1.DebugState.getInstance().saveConfig();
        if (saved) {
            vscode.window.showInformationMessage('Debug configuration saved');
        }
    }));
    // Load Configuration
    context.subscriptions.push(vscode.commands.registerCommand('cobolDebug.loadConfiguration', async () => {
...
async function generateDebugVersion() {
    ...
    const answer = await vscode.window.showInformationMessage(`Ready to generate debug version:\n\n` +
        ...
        `Continue?`, 'Generate', 'Show Details', 'Cancel');
 ...
 */
async function watchVariable(mode) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        return;
    }
    ...
    if (!config) {
        ...
        const program = cobolParser_1.CobolParser.parse(content);
    ...
    state.addSinglePointWatch({
        ...
        mode: mode,
        showRelatedVars: mode === types_1.DebugMode.BEFORE_AFTER
    });
...
async function trackVariableAll() {
    ...
    const occurrences = cobolParser_1.CobolParser.findVariableOccurrences(lines, selectedText, program.procedureDivisionStart);
    ...
    if (occurrences.length > warnThreshold) {
        const answer = await vscode.window.showWarningMessage(`⚠️ Warning: ${selectedText} found in ${occurrences.length} locations.\n` +
            `This will generate ~${occurrences.length * 2} DISPLAY statements.\n\n` +
            `Continue?`, 'Continue Anyway', 'Cancel');
...
async function traceParagraph() {
    ...
    const paragraph = cobolParser_1.CobolParser.getParagraphAtLine(program.paragraphs, cursorLine);
    if (!paragraph) {
...
async function debugAllVariablesInParagraph() {
    ...
    for (let i = paragraph.startLine; i <= paragraph.endLine; i++) {
        const line = lines[i];
        const codeLine = cobolParser_1.CobolParser.getCodeArea(line);
        ...
        if (words) {
            for (const word of words) {
                ...
                if (allVars.some(v => v.toUpperCase() === word.toUpperCase())) {
                    if (!cobolParser_1.CobolParser.isCobolKeyword(word)) {
    ...
    if (detectedVars.size === 0) {
        vscode.window.showInformationMessage(`No variables detected in paragraph ${paragraph.name}`);
        return;
    }
    ...
    for (const varName of selected) {
        if (scope.value === 'all') {
            ...
            const occurrences = cobolParser_1.CobolParser.findVariableOccurrences(lines, varName, program.procedureDivisionStart);
            if (occurrences.length > 0) {
                state.addVariableTracking({
                    variable: varName,
                    trackAllOccurrences: true,
            ...
            const occurrences = cobolParser_1.CobolParser.findVariableOccurrences(lines, varName, program.procedureDivisionStart).filter(occ => occ.line >= paragraph.startLine && occ.line <= paragraph.endLine);
            if (occurrences.length > 0) {
                state.addVariableTracking({
                    variable: varName,
    ...
    if (scope.value === 'all') {
        vscode.window.showInformationMessage(`Added tracking for ${selected.length} variable(s) in entire program`);
    }
    else {
        vscode.window.showInformationMessage(`Added tracking for ${selected.length} variable(s) in paragraph ${paragraph.name}`);
    }
}
/**
 * Debug variables in selection
...
async function debugVariablesInSelection() {
    ...
    const mode = await vscode.window.showQuickPick([
        { label: 'BEFORE only', value: types_1.DebugMode.BEFORE },
        { label: 'AFTER only', value: types_1.DebugMode.AFTER },
        { label: 'BEFORE/AFTER', value: types_1.DebugMode.BEFORE_AFTER }
    ], {
    ...
    for (const varName of selected) {
        state.addSinglePointWatch({
            ...
            line: startLine,
            mode: mode.value,
            showRelatedVars: mode.value === types_1.DebugMode.BEFORE_AFTER
        });
    }
    vscode.window.showInformationMessage(`Added ${selected.length} variable(s) to debug at line ${startLine + 1}`);
}
/**
 * Remove from debug
 */
...
async function loadConfiguration() {
    ...
    const content = document.getText();
    const program = cobolParser_1.CobolParser.parse(content);
    const loaded = await debugState_1.DebugState.getInstance().loadConfig(program.programName);
    if (loaded) {
